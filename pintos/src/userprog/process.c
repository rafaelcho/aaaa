...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  // ----------------------------------------
  char * cur_ptr = NULL, * next_ptr = NULL;
  int  arg_len = 0, align_len = 0;
  uint8_t word_align = 0;

  cur_ptr = strtok_r(file_name, " ", &next_ptr);

  while(cur_ptr){
    cur_ptr = strtok_r(NULL, " ", &next_ptr);
    if(cur_ptr != NULL)
      argv[argc++] = cur_ptr;
  }
  ...
  align_len = 4 - arg_len % 4;

  for(i = 0; i < align_len; i++){
    *esp = *esp - 1;
    memcpy(*esp, &word_align, 1);
  }
  ...
  *((int*)*esp) = 0;
  ...
  //memcpy(temp, hello, strlen(hello)+1);
  //hex_dump(0, temp, 10, 1);
  //*esp = *esp - strlen(hello) - 1;
  //memcpy(*esp, temp, strlen(temp)+1);
  //memcpy(*esp, hello, strlen(hello)+1);
 
  //*esp = *esp - (arg_len + align_len);
  ...
  cur_ptr = file_name + strlen(file_name) + 1;

  for(i = 0; i < cnt; i++){
    memcpy(*esp, cur_ptr, strlen(cur_ptr) + 1);
    *esp += strlen(cur_ptr) + 1;
...
*/
  ...
  hex_dump(PHYS_BASE-60, *esp, 100, true);
  // -------------------------------------------
